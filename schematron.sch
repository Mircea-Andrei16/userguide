...
<sch:schema xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    lkhjnuiiihhhhhhhhhhhhhhhhhhhhhhhhhhh<sch:pattern id="avoidPercent" is-a="avoidWordInContext">
    <sch:param name="context" value="*"/>
    <sch:param name="word" value="percent"/>
    <sch:param name="message"
      value="The '%' symbol should be used instead of spelling out 'percent'."/>
  </sch:pattern>
  ...
  <sch:pattern id="curencySymbolShouldStayBeforeNumber" is-a="avoidRegExpPattern">
    ...
    <sch:param name="message"
      value="The currency symbol should be placed before the number, not after."/>
  </sch:pattern>
  ...
  <sch:pattern id="avoidWordInContext" abstract="true">
    <sch:title>Issue a warning if a word or a phrase appears inside a specified element</sch:title>
    <sch:p>This pattern allows you to advise users not to use a specific word in an element, or in
      multiple elements, if you separate them using '|'. The check is case insensitive.</sch:p>
    <parameters xmlns="http://oxygenxml.com/ns/schematron/params">
      <parameter>
        <name>context</name>
        <desc>Specifies the context we will verify to not contain a specified word. You can specify
          multiple elements if you separate them using a pipe character, for example title|p will
          check both title and p elements. </desc>
      </parameter>
      <parameter>
        <name>word</name>
      ...
      <parameter>
        <name>message</name>
        <desc>The message the end user will see when the specified word appears in the specified
          element.</desc>
      </parameter>
    </parameters>
    <sch:rule context="$context">
      <sch:report test="./text()[matches(., '(^|\s)$word([\s,;.:!?]|$)', 'i')]" role="warn">$message
      </sch:report>
    </sch:rule>
  </sch:pattern>
  <sch:pattern id="avoidEndFragment" abstract="true">
    <sch:title>Issue a warning if an element ends with a specified fragment or character</sch:title>
    <sch:p>This pattern allows you to advise users not to use a specific end sequence to end an
      element.</sch:p>
    <parameters xmlns="http://oxygenxml.com/ns/schematron/params">
      <parameter>
        <name>element</name>
    ...
    <sch:rule context="$element">
      <sch:assert test="not(ends-with(normalize-space(.), '$fragment'))" role="warn">$message
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern id="restrictNumberOfChildren" abstract="true">
    ...
    <sch:p>Check the number of children of an element in a parent element to be between specified
      limits.</sch:p>
    <parameters xmlns="http://oxygenxml.com/ns/schematron/params">
      <parameter>
    ...
    <sch:rule context="$parentElement">
      ...
      <sch:assert test="$children &lt;= $max" role="warn">$message </sch:assert>
      <sch:assert test="$children &gt;= $min" role="warn">$message </sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern id="restrictChildrenElements" abstract="true">
    ...
    <sch:rule context="$parentElement/*">
      ...
      <sch:report test="child::*/local-name() = $elements" role="warn">$message </sch:report>
    </sch:rule>
  </sch:pattern>
  <sch:pattern id="avoidRegExpPattern" abstract="true">
    ...
    <sch:rule context="$context">
      <sch:report test="./text()[matches(., '$regexp', '$flags')]" role="warn">$message
      </sch:report>
    </sch:rule>
  </sch:pattern>
</sch:schema>
